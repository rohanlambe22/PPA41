...
void man(char *name)
{
    ...
    {
        printf("Description :Used to read the data froem regular file \n");
    ...
    else if(strcmp(name,"turncate")== 0)
    {
        ...
        printf("Usage : trncate File_name\n");
    }
...
void DisplayHelp()
{
    ...
    printf("tuencate: To Remove all data from file\n");
...
void createDILB()
{
    ...
    printf("DILB creadted successfully \n");
}
...
int ReadFile(int fd , char *arr,int isize)
{
    ...
    if(read_size < isize)
    {
        memcpy(arr,(UFDTArr[fd].prtfiletable->ptrinode->Buffer) + (UFDTArr[fd].prtfiletable->readoffset),read_size); 

        UFDTArr[fd].prtfiletable->readoffset = UFDTArr[fd].prtfiletable->readoffset + read_size;
    }
    else
    {
        memcpy(arr,(UFDTArr[fd].prtfiletable->ptrinode->Buffer)+(UFDTArr[fd].prtfiletable->readoffset),isize);
...
int WriteFile(int fd, char *arr,int isize)
{
    ...
    if(((UFDTArr[fd].prtfiletable->ptrinode->permission)!= WRITE)&&((UFDTArr[fd].prtfiletable->ptrinode->permission)!= READ + WRITE))
    {
        return -1;
    }
    ...
    memcpy((UFDTArr[fd].prtfiletable->ptrinode->Buffer)+ (UFDTArr[fd].prtfiletable->writeoffset),arr,isize);

    (UFDTArr[fd].prtfiletable->writeoffset)= (UFDTArr[fd].prtfiletable->writeoffset)+isize;

    (UFDTArr[fd].prtfiletable->ptrinode->FileActualSize) = (UFDTArr[fd].prtfiletable->ptrinode->FileActualSize)+ isize;
...
int  OpenFile(char *name , int mode)
{
    ...
    UFDTArr[i].prtfiletable = (PFILETABLE)malloc(sizeof(FILETABLE));
    if (UFDTArr[i].prtfiletable == NULL)  return -1;
...
int LseekFile(int fd ,int size, int from)
{
    ...
    if((UFDTArr[fd].prtfiletable->mode == READ)||(UFDTArr[fd].prtfiletable->mode)== READ + WRITE)
    {
        if(from == CURRENT)
        {
            if(((UFDTArr[fd].prtfiletable->readoffset)+ size) > UFDTArr[fd].prtfiletable->ptrinode->FileActualSize ) return -1;
            if(((UFDTArr[fd].prtfiletable->readoffset)+ size) < 0) return -1;
            (UFDTArr[fd].prtfiletable->readoffset) = (UFDTArr[fd].prtfiletable->readoffset)+ size;
        }
        else if( from == START)
        {
            if(size > (UFDTArr[fd].prtfiletable->ptrinode->FileActualSize)) return -1;
            if(size < 0) return -1;
            (UFDTArr[fd].prtfiletable->readoffset)= size;
        }
        else if(from == END)
        {
            if(size > (UFDTArr[fd].prtfiletable->ptrinode->FileActualSize)+ size > MAXFILESIZE) return -1;
            if(((UFDTArr[fd].prtfiletable->readoffset)+ size) < 0) return-1;
    ...
    else if(UFDTArr[fd].prtfiletable->mode == WRITE)
    {
        if(from == CURRENT)
        {
            if(((UFDTArr[fd].prtfiletable->writeoffset)+size)>MAXFILESIZE) return-1;
            if(((UFDTArr[fd].prtfiletable->writeoffset)+size)<0) return-1;
        ...
        {
            if((UFDTArr[fd].prtfiletable->ptrinode->FileActualSize)+ size > MAXFILESIZE) return -1;
            if(((UFDTArr[fd].prtfiletable->writeoffset)+ size) < 0) return -1;
...
int main()
{
   ...
   while(1)
   {
        ...
        if(count == 1)
        {
            ...
            {
                system("cls");    // system("clear"); for linix 
                continue;
            }
            else if(strcmp(command[0],"help") == 0)
            {
                DisplayHelp();
                continue;     // used for fast processing 
            }
        ...
        else if(count == 3)
        {
            if(strcmp(command[0],"create") == 0)
            {
                ...
                if(ret >= 0)
                   printf("File is successfully crested with file Descriptor : %d\n",ret);
                if(ret == -1)
                   printf("ERROR : Incorrect paramentrs \n");
                if(ret == -2)
                   printf("ERRPR : There is no inodes \n");
                if(ret == -3)
            ...
            if(strcmp(command[0],"read") == 0)
            {
                ...
                if(fd == -1)
                {
                    printf("ERROR : Incorrect paraments\n");
                    continue;
                }
                ...
                if(ret == -1)
                   printf("ERROR : File not exsiting\n");
                if(ret == -2)
                   printf("ERRPR :Permission denied\n");
                if(ret == -3)
